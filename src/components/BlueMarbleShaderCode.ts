...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 m = vec2(0.);
	if( length(iMouse) > 0.0 ) m = iMouse.xy/iResolution.xy*3.14;
    m-=.5;
    ...
    if (tmm.x<0.)col = vec3(0,0,0);
    else {
        vec3 nor=(ro+tmm.x*rd)/2.;
        nor = reflect(rd, nor);        
        float fre = pow(.5+ clamp(dot(nor,rd),0.0,1.0), 5. )*1.3;
        col += vec3(0,0,0) * fre;
