...
{
    return -1.0 + 2.0 * noise(x);
}

// Star field replacement for iChannel1
vec3 getStars(vec3 dir) {
    vec3 q = dir;
    float stars = 0.0;
    for(int i=0; i<3; i++) {
        vec3 p = q * (100.0 + float(i) * 50.0);
        float n = hash(floor(p));
        if(n > 0.99) {
            float size = hash(floor(p) + 0.5);
            stars += smoothstep(0.1, 0.0, length(fract(p) - 0.5)) * size;
        }
    }
    return vec3(stars);
}

// Planet texture replacement for iChannel2
vec3 getPlanetTex(vec2 uv) {
    float n = noise(vec3(uv * 8.0, 0.0)) * 0.5 + 0.5;
    n += noise(vec3(uv * 16.0, 0.5)) * 0.25;
    return vec3(n);
}
...
float Map(vec3 pos)
{
    ...
    float fft = 0.0; // Audio placeholder
...
float EarthHeight(vec3 pos)
{
    ...
    vec3 te = getPlanetTex(coord) + getPlanetTex(coord*3.0) * .3;
    float landLerp = smoothstep( 0.45, 0.5, te.x);
    vec3 albedo = mix( vec3(0.1, 0.2, 0.45), (vec3(0.055, 0.275, 0.0275) + 0.45*te + te*te*0.5*getPlanetTex(2.0*coord.xy))*0.4, landLerp );
...
vec3 BackgroundInner(vec3 pos, vec3 dir, bool enableSun, out bool sphereHit, out vec3 spherePos, out vec3 sphereNormal, out vec3 reflectivity)
{
    ...
    vec3 stars = getStars(dir);
    color *= stars * 4.0;
    color += pow(stars, vec4(4.0).rgb) * 0.5;
    ...
    if(t >= 0.0)
    {
        ...
        if(type == 0)
        {
            float offset = getPlanetTex(coord).r * .005;
            ...
            vec3 fire = (getPlanetTex(coord*5. + time*.02) +
                         getPlanetTex(coord*1. + time*.006)
                        ) * vec3(3,1,1) * .5;

            vec3 albedo = getPlanetTex(coord + vec2(offset)) * .25 -
                          getPlanetTex(coord*7.0) * .1;
            color = albedo * max(0., dot(sphereNormal, sunDir)) * sunColor + fire * pow(1.0-height,16.);
        }
        else if(type == 2)
        {
            vec3 te = getPlanetTex(coord) + getPlanetTex(coord*3.0) * .3;
            float offset = 0.0 + getPlanetTex(coord).x*.003;
            vec3 albedo = (getPlanetTex(coord*vec2(.4,0)+vec2(offset,0))-.5)*.7 + .4;
            albedo += getPlanetTex(coord*1.0) * .2;
            albedo += getPlanetTex(coord*16.0) * .075;
        ...
        else if(type == 1)
        {
            ...
            float q = (getPlanetTex(coord+vec2(0,time*.02)).x +
                       getPlanetTex(coord*2.0+vec2(0,time*.013)).x) * .5;
